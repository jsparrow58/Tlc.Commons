...
namespace Tlc.Web
{
    public static class WebRequest
    {
        ...
        /// <returns></returns>
        public static async Task<HttpWebResponse> GetAsync(string url, Action<HttpWebRequest> configRequest = null, string bearerToken = null)
        {
        ...
        /// <param name="bearerToken">在特殊情况下，请求需要发送bearerToken,以表明请求身份</param>
        /// <returns></returns>
        public static async Task<HttpWebResponse> PostAsync(string url, object content = null, ContentSerializers sendType = ContentSerializers.Json,
            ContentSerializers returnType = ContentSerializers.Json, Action<HttpWebRequest> configRequest = null, string bearerToken = null)
        {
            var request = System.Net.WebRequest.CreateHttp(url);
            request.Method = WebRequestMethods.Http.Post;
            request.ContentType = sendType.ToMimeString();
            request.Accept = returnType.ToMimeString();
            if (!bearerToken.IsNullOrWhiteSpace()) request.Headers.Add(HttpRequestHeader.Authorization, $"Bearer {bearerToken}");
